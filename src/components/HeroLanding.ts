  ...
  return `
    ...
    >
      ...
      <div id="landing-ctas">
        ...
        <a
          id="landing-hero-android"
          href="https://play.google.com/store/apps/details?id=com.gustavodev25.controlarapp&hl=pt_BR"
          target="_blank"
          rel="noopener noreferrer"
          aria-label="Baixar na Google Play"
        >
          <svg width="14" height="17" viewBox="0 0 512 512" fill="currentColor">
...
export function attachHeroLandingListeners() {
  ...
  // App Store / Play Store button morphs
  const iosHeroBtn = document.getElementById('landing-hero-ios');
  if (iosHeroBtn) {
    setupHeroWhiteButtonMorph(iosHeroBtn);
  }
  const androidHeroBtn = document.getElementById('landing-hero-android');
  if (androidHeroBtn) {
    setupHeroWhiteButtonMorph(androidHeroBtn);
  }
